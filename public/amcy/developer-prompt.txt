...
## Dedicated quotation pipeline

- Provide a full-page quotation register with search, status filtering, pipeline value, awaiting-response count, win rate, converted count, and CSV export.
- Provide a full-page quote builder; do not use a popup or browser dialog.
- Let the administrator select a saved customer or enter customer name, phone, and email manually.
- Require only a customer name and at least one item. Allow product, quantity, editable unit price, discount, configured tax, validity date, reference, and terms.
- Save draft, sent, accepted, declined, expired, and converted status in the main Realtime Database.
- Expired status must be derived automatically for unaccepted draft or sent quotes after their validity date.
- Generate configurable sequential quotation numbers independently from invoices.
- Print a professional branded quotation containing customer details, validity, line items, totals, and terms.
- Convert an eligible quote into the point-of-sale checkout with its customer, items, quoted prices, and discount preserved.
- Conversion must recheck current stock and only mark the quotation converted after the invoice transaction succeeds.
- Save the linked quotation ID on the invoice and the linked invoice ID on the quotation.
...
- Use dedicated pages for operational forms; do not use modal dialogs for Add Product, Stock Movement, Supplier, Purchase Order, Quotation, Customer, Invoice Detail, or Security DZ
